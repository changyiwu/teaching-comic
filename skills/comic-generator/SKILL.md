...
- 每個泡的**內部空白**至少 240×110 像素（以 540×675 的單格為基準），中文才排得下。
- **`shout` 鋸齒泡的內部空白要再放大一圈，至少 300×150 像素**：鋸齒的尖角會往內吃掉可用區，實測若照一般泡的尺寸生圖，量到的內緣只剩 160×114，數學式與長句就排不下。生圖提示要對 shout 泡額外加註 `oversized`／`extra large`，例如 `an oversized jagged spiky burst balloon, much larger than the other balloons, with a wide clean empty interior`。
...
座標以各面板左上角為原點；每格範圍為寬 540、高 675。`x`、`y`、`w`、`h` 要給泡的**完整內緣**（從 `_normalized.png` 量到的白色可用區），**不要預先內縮**。腳本自己會退縮排：一般泡左右各退 `w` 的 9～15%（下限 18px）、上下各退 `h` 的 14～20%（下限 14px），`shout` 再加碼到至少 14%／18%，鋸齒與虛線邊已經由這層 padding 讓開。

先自行內縮（例如填「最大內接矩形」）等於縮兩次，可用寬高只剩一半，字級掉到 `min_font_size` 仍排不下，腳本會直接報 `Text does not fit`。

`text` **支援 `\n` 強制換行**（腳本以 GDI+ `MeasureString`／`DrawString` 排字，原生吃換行字元）。自動換行只看寬度，會把數學式從中間拆開（例如 `4×(-2)` 和 `= -8` 被拆到兩行），排數學式或想控制斷句時，直接在 `text` 裡寫死換行點：

```json
{ "panel": 3, "type": "shout", "x": 40, "y": 60, "w": 160, "h": 114,
  "text": "2 天前是\n4×(-2) = -8" }
```

同一個原因，窄泡裡的等號用**半形 `=`**，比全形 `＝` 省下約一個全形字寬，常常就是能不能排成一行的關鍵。
