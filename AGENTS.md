...
- 對白座標一律從實際的 `_normalized.png` 量測泡的內緣；`w`／`h` 給**完整內緣**，不要預先內縮（腳本自己會加 padding，先內縮會縮兩次而報 `Text does not fit`）
- `shout` 鋸齒泡內部空白需求比一般泡大（至少 300×150 px），生圖提示需加註 `oversized`，避免鋸齒尖角吃掉可用空間
- `text` 支援 `\n` 強制換行，排數學式時直接寫死換行點避免被自動折行拆散；窄泡等號用半形 `=` 省字寬
